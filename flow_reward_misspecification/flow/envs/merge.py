        ...
        for i, rl_id in enumerate(self.rl_veh):
            if rl_id not in self.k.vehicle.get_rl_ids():
                ...
                acc_controller_actions.append(
                    self.k.vehicle.get_acc_controller(rl_id).get_controller_accel(self)
                )
...
class MergePOEnvAccel(MergePOEnv):
    def compute_reward(self, rl_actions, **kwargs):
        ...
        if self.env_params.evaluate:
            ...
            if self.local_reward == "local":
                rl_veh = self.rl_veh
                cost1 = rewards.local_desired_velocity(
                    self, self.rl_veh, fail=kwargs["fail"]
                )
            elif self.local_reward == "partial_first":
                cost1 = rewards.local_desired_velocity(
                    self, self.rl_veh[:3], fail=kwargs["fail"]
                )
            elif self.local_reward == "partial_last":
                cost1 = rewards.local_desired_velocity(
                    self, self.rl_veh[-3:], fail=kwargs["fail"]
