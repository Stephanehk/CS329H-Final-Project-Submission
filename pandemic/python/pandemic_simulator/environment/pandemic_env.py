...
class PandemicGymEnv(gymnasium.Env):
    ...
    def step_keep_obs_obj(self, action: int) -> Tuple[PandemicObservation, float, bool, Dict]:
        cur_stage = self.stages[self.stage_idx]
        stage = cur_stage.stage
        actual_stage = self._last_observation.stage[-1, 0, 0]
        if (
        ...
        if self._use_safe_policy_actions:
            obs,obs_np, reward, terminated, truncated, info = self._step_keep_obs_obj(safe_policy_action)
        else:
            obs,obs_np, reward, terminated, truncated, info = self._step_keep_obs_obj(action)

        info[self._safe_policy] = safe_policy_action

        return obs,obs_np, reward, terminated, truncated, info


    def _step_keep_obs_obj(self, action: int) -> Tuple[PandemicObservation, float, bool, Dict]:
        # assert self.action_space.contains(action), "%r (%s) invalid" % (action, type(action))
    ...
    def reset_keep_obs_obj(self, *, seed=None, options=None):
        self._pandemic_sim.reset()
        self._last_reward = 0.0
        self._last_true_reward = 0.0
        if self._done_fn is not None:
        ...
        if self.four_start:
            ...
            return self._last_observation,self._obs_with_history, {}
