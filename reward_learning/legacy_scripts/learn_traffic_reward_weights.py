...
# Placeholder: you should implement or import create_traffic_reward
...
def load_rollout_data(rollout_dir: str, policy_names: List[str], n_trajectories_per_policy: int = 10) -> List[Dict[str, Any]]:
    ...
    data_filename = f"traffic_{policy_names_str}_rollout_data.pkl"
...
def learn_reward_weights_from_preferences(rollout_dir: str, policy_names: List[str], n_trajectories_per_policy: int = 10, n_preference_pairs: int = 5000, use_linear_model: bool = True, debug: bool = False):
    ...
    reward = create_traffic_reward(debug=debug)  # You must implement this
    ...
    for transition in rollout_data:
        ...
        if transition["done"]:  # Adjust for your trajectory length
    ...
    for traj1_idx in range(len(trajectories)):
        for traj2_idx in range(traj1_idx + 1, len(trajectories)):
            ...
            # Stochastic preferences (uncomment if needed)
    ...
    # X = np.load(save_dir / "traffic_preference_X.npy")
    # y = np.load(save_dir / "traffic_preference_y.npy")

    X = np.array(X)
    y = np.array(y)

    # np.save(save_dir / "traffic_preference_X.npy", X)
    # np.save(save_dir / "traffic_preference_y.npy", y)
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            print(f"Epoch [{epoch+1}/{n_epochs}], Loss: {total_loss/len(dataloader):.4f}, LR = {scheduler.get_last_lr()}")
    ...
    save_path = save_dir / f"{prefix}traffic_preference_weights_{policy_names_str}.json"
...
if __name__ == "__main__":
    ...
    policy_names = ["traffic_base_policy", "2025-06-17_16-14-06", "2025-06-24_13-51-42"] #, "2025-06-17_16-14-06", "traffic_base_policy"
    debug = False
    # Use preference learning instead of direct regression
